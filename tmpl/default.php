...
<div class="container">
    <div id="article-container" class="row">
        <?php foreach ($articles as $index => $article) : ?>
            <?php
                ...
                // The schema.org thumbnailUrl is intentionally absolute (required by the
                // JSON-LD/SEO spec), but reusing it as-is for this VISIBLE <img> hardcodes
                // whichever host generated it (breaks across dev/staging/prod). Convert it
                // to a root-relative path before using it here.
                if ($schemaImage !== '' && preg_match('#^https?://[^/]+(/.*)$#i', $schemaImage, $hostMatch)) {
                    $schemaImage = $hostMatch[1];
                }
    ...
    <div class="row d-grid gap-1 col-6 mx-auto">
        <?php if ($totalArticles > $loadMoreStep) : ?>
                ...
                <a id="seeMoreLink" href="/bitacora" class="btn btn-secondary btn-lg mt-4 mb-5 text-center" style="display:none;" target="_blank">¡Visíta nuestra bitácora!</a>
        <?php endif; ?>
    </div>
